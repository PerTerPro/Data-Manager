...
//        public void Analytics(ProductEntity pt, HtmlDocument doc, String detailUrl,
//            Configuration conf, string domain)
//        {
//            try
//            {
...
//                pt.Categories = ParseCategories(doc, conf.CategoryXPath, domain, conf.RemoveLastItemClassification,
//                    detailUrl);
//                pt.OriginPrice = (int) ParseOriginPrice(doc, conf.OriginPriceXPath, conf.CheckPrice);
//                pt.Price = ParsePrice(doc, conf.PriceXPath, conf.CheckPrice, new Uri(detailUrl), null, _con);
...
//                pt.VideoDescHtml = GetDesc(doc,
//                    conf.VideoXpath.Split(Common.arSplitToList, StringSplitOptions.RemoveEmptyEntries).ToList());
//                pt.SpecsDescHtml = GetDesc(doc,
//                    conf.SpecsXPath.Split(Common.arSplitToList, StringSplitOptions.RemoveEmptyEntries).ToList());
//                pt.FullDescHtml = GetDesc(doc,
//                    conf.FullDescXPath.Split(Common.arSplitToList, StringSplitOptions.RemoveEmptyEntries).ToList());
//                pt.ShortDescHtml = GetDesc(doc, conf.ShortDescriptionXPath);
//            }
//            catch (Exception ex)
//            {
//                Exception ex01 = new Exception(string.Format("Company: {0} Product: {1} Url: {2}: {3}",
//                    pt.CompanyId, pt.ID, pt.DetailUrl, ex.Message + ex.StackTrace));
//                throw ex01;
//            }
//        }
...
//        private long ParsePrice(HtmlDocument doc, List<string> list, bool checkPrice, Uri rootUri, List<string> regexPrice)
//        {
//            long Price = 0;
...
//                        if (list[0].Trim() != "")
//                        {
//                            var node_price = doc.DocumentNode.SelectNodes(list[0]);
//                            if (node_price != null)
//                            {
...
//                        if (list[1].Trim() != "")
//                        {
//                            var node_price = doc.DocumentNode.SelectNodes(list[1]);
//                            if (node_price != null)
//                            {
//                                int K = node_price.Count;
...
//                    break;
//                case "www.trananh.vn":
//                    Price = ParsePriceTranAnh(doc, list);
//                    break;
//                default:
...
//                        if (list[i].Trim() != "")
//                        {
//                            var node_price = doc.DocumentNode.SelectNodes(list[i]);
...
//                                            {
//                                                s =
//                                                    QT.Entities.Common.GetTextInNode(doc, list[i])
//                                                        [0];
//                                            }
//                                        }
//                                    }
...
//        private long ParseOriginPrice(GABIZ.Base.HtmlAgilityPack.HtmlDocument doc, List<string> list,  bool checkPrice)
//        {
//            int originPrice = 0;
//            if (list != null
//                && list.Count > 0
//                && list[0].Trim() != "")
//            {
//                var nodePrice = doc.DocumentNode.SelectNodes(list[0]);
//                if (nodePrice != null)
//                {
//                    int K = nodePrice.Count;
//                    for (int j = 0; j < K; j++)
//                    {
//                        string s = Tools.removeHTML(nodePrice[j].InnerText);
