...
namespace WSS.Core.Crawler.CrlProduct
{
    public static class ControlCmpCrlw
    {
        ...
        static readonly RedisCompanyWaitCrawler RedisCompanyWaitCrawler = RedisCompanyWaitCrawler.Instance();
        public static void PushCmp()
        {
            ...
            foreach (var tuple in tupleSetup)
            {
                ...
                {
                    var tblCmpFn = sqldb.GetTblData("prc_Compnay_GetCmpToPushCrl", CommandType.StoredProcedure, new[]
                    {
                    ...
                    {long companyId = Common.Obj2Int64(variable["ID"]);
                        if (!RedisCompanyWaitCrawler.CheckRunningCrawler(companyId))
                        {
                            producer.PublishString(new JobCompanyCrawler()
                            {
                                CheckRunning = true,
                                CompanyId = companyId
                            }.GetJSon());Log.Info(string.Format("Pushed for cmp: {0}", companyId));
                        }
                    }}
            }
            return;
