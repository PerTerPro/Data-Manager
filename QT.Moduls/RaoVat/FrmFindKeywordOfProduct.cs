...
{
    public partial class FrmFindKeywordOfProduct : Form
    {
        log4net.ILog log = log4net.LogManager.GetLogger(typeof(FrmExtractionKeyWord));
        private bool IsStop;

        public FrmFindKeywordOfProduct()
        {
            InitializeComponent();
        }
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                thread = new Thread(() => MethodExtractKeyWordToMongo(query, update, field
                    , limit,1, fieldIncreate, iCreateMent ));
                thread.Start();
            ...
            , int iIncr)
        {
            ...
            while (true)
            {
                try
                {
                    ...
                    SqlDb sqldb = new SqlDb(QT.Entities.Server.ConnectionStringCrawler);
                    MongoDbRaoVat mongoDb = new MongoDbRaoVat();
                    foreach (var item in mongoDb.GetListExtractKeyWord(queryFind, limit, 1).Result)
                    {
                        iCountProcessed++;
                        if (item.Contains(field))
                        {
                            ...
                            foreach (var itemKeyWord1 in ieKeyWord)
                            {
                                ...
                                {
                                    string itemKeywords = itemKeyWord1.ToString();
                                    BsonDocument KeyWordata = mongoDb.GetKeywordByNameKeyWord(itemKeywords).Result;
                                    if (KeyWordata != null 
                                        && KeyWordata.Contains("_id") 
                                        && KeyWordata.Contains("score_gram"))
                                    {
                                        string id = KeyWordata["_id"].ToString();
                                        int iCount = KeyWordata["score_gram"].AsInt32;
                                        if (!dicKeyWordAndStore.ContainsKey(id)) dicKeyWordAndStore.Add(id, iCount);
                                    }
                                }
                                catch (Exception ex)
                                {
                            ...
                            //Lấy ra dữ liệu cần insert vào db.
                            var sortedDict = dicKeyWordAndStore.OrderByDescending(entry => entry.Value).Take(20).ToDictionary
                                (pair => pair.Key, pair => pair.Value);
                            mongoDb.UpdateKeyWordForProduct(item["_id"].AsObjectId, sortedDict.Keys.ToArray());
                        }
                        mongoDb.UpdateLoadKeyWord(item["_id"].AsObjectId, queryUpdate, 1);
                        WriteLog(string.Format("Processed product:{0}:{1}:{2}", iCountProcessed, item["_id"].AsObjectId.ToString(), item["url"].AsString));
                        Thread.Sleep(1000);
                    }
                }
                catch (ThreadAbortException ex)
  ...
  where  PushToMongo=0", CommandType.Text,
        ...
        {
            txtQuery.Text = @"{$or:[{'is_wait_keyword':{$exists:false}},{'is_wait_keyword':false}]}";
            txtUpdate.Text = @"{$set:{'is_wait_keyword':true}}";
            txtField.Text = "name";
        }
