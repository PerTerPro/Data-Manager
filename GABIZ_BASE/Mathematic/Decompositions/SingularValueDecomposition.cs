        ...
        public SingularValueDecomposition(double[,] value, bool computeLeftSingularVectors, bool computeRightSingularVectors, bool autoTranspose, bool inPlace)
        {
            ...
            for (int k = 0; k < System.Math.Max(nct, nrt); k++)
            {
                if (k < nct)
                {
                    ...
                    for (int i = k; i < m; i++)
                    {
                        s[k] = GABIZ.Base.Mathematic.Tools.Hypotenuse(s[k], a[i, k]);
                    }
                ...
                if (k < nrt)
                {
                    ...
                    for (int i = k + 1; i < n; i++)
                    {
                        e[k] = GABIZ.Base.Mathematic.Tools.Hypotenuse(e[k], e[i]);
                    }
                    ...
                    case 1:
                        {
                            ...
                            for (int j = p - 2; j >= k; j--)
                            {
                                double t = GABIZ.Base.Mathematic.Tools.Hypotenuse(s[j], f);
                                double cs = s[j] / t;
                                double sn = f / t;
                                s[j] = t;
                                if (j != k)
                    ...
                    case 3:
                        {
                            ...
                            for (int j = k; j < p - 1; j++)
                            {
                                double t = GABIZ.Base.Mathematic.Tools.Hypotenuse(f, g);
                                double cs = f / t;
                                double sn = g / t;
                                if (j != k) e[j - 1] = t;
                                f = cs * s[j] + sn * e[j];
                                ...
                                t = GABIZ.Base.Mathematic.Tools.Hypotenuse(f, g);
                                cs = f / t;
                                sn = g / t;
                                s[j] = t;
                                f = cs * e[j] + sn * s[j + 1];
