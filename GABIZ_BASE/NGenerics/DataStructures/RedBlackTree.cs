...
namespace GABIZ.Base.NGenerics.DataStructures
{
	...
	public class RedBlackTree<TKey, TValue> : IVisitableDictionary<TKey, TValue>
	{
		...
		private RedBlackTreeNode<TKey, TValue> root = null;
		private int count = 0;
		private IComparer<TKey> comparerToUse = null;
		...
		public int CompareTo(object obj)
		{
			...
			{
				RedBlackTree<TKey, TValue> rb = obj as RedBlackTree<TKey, TValue>;
				return this.Count.CompareTo(rb.Count);
			}
			else
			{
        ...
        private RedBlackTreeNode<TKey, TValue> InsertNode(RedBlackTreeNode<TKey, TValue> node, TKey key, TValue value)
        {
            ...
            else if (comparerToUse.Compare(key, node.Key) != 0)
            {
                ...
                if (RedBlackTreeNode<TKey, TValue>.IsRed(node[direction]))
                {
                    ...
                    {
                        if (RedBlackTreeNode<TKey, TValue>.IsRed(node[direction][direction]))
                        {
        ...
        private void VisitNode(RedBlackTreeNode<TKey, TValue> node, OrderedVisitor<KeyValuePair<TKey, TValue>> visitor)
        {
            if (node != null)
            {
                KeyValuePair<TKey, TValue> pair = new KeyValuePair<TKey, TValue>(node.Key, node.Value);
        ...
        public ICollection<TKey> Keys
        {
            get {
                ...
                InOrderVisitor<KeyValuePair<TKey, TValue>> inOrderVisitor = new InOrderVisitor<KeyValuePair<TKey, TValue>>(visitor);
                
                this.DepthFirstTraversal(inOrderVisitor);
        ...
        public bool Remove(TKey key)
        {
            if (root != null)
            {
                RedBlackTreeNode<TKey, TValue> startNode = new RedBlackTreeNode<TKey, TValue>(default(TKey), default(TValue));
                ...
                while (childNode[direction] != null)
                {
                    ...
                    if ((RedBlackTreeNode<TKey, TValue>.IsBlack(childNode)) && (RedBlackTreeNode<TKey, TValue>.IsBlack(childNode[direction])))
                    {
                        ...
                        else if (RedBlackTreeNode<TKey, TValue>.IsBlack(childNode[direction]))
                        {
                            ...
                            if (sibling != null)
                            {
                                if ((RedBlackTreeNode<TKey, TValue>.IsBlack(sibling.Left)) && (RedBlackTreeNode<TKey, TValue>.IsBlack(sibling.Right)))
                                {
